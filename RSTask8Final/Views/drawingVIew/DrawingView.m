...
@property (weak, nonatomic) CAShapeLayer *drawingLayer;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        _colorQueue = [[NSMutableArray alloc] init];
        _drawingSpeed = 1.0 / 6.0;
...
- (void)drawRect:(CGRect)rect {
    NSLog(@"DrawRect");
    for (int i = 0; i < 3 - self.colorQueue.count; i++) {
        ColorButton *defaultBlackButton = [[ColorButton alloc] initWithFrame:CGRectNull Color:[UIColor colorNamed:@"RSBlack"] andColorTypeEnum:colorTypeBlack];
        [self.colorQueue addObject:defaultBlackButton];
    }
    NSLog(@" picture.type before drawRect %ld", (long)self.pictureType);
    ...
    switch (self.pictureType) {
        case 1:
            ...
            NSLog(@"Case planet");
            [self preparing];
            self.drawingLayer.path = [self drawPlanet].CGPath;
            [self.delegate justDrawing];
            break;
        case 2:
            // HEAD
            [self preparing];
            self.drawingLayer.path = [self drawHead].CGPath;
            [self.delegate justDrawing];
            break;
        case 3:
            // TREE
            [self preparing];
            self.drawingLayer.path = [self drawTree].CGPath;
            [self.delegate justDrawing];
            break;
        case 4:
            // LANDSCAPE
            [self preparing];
            self.drawingLayer.path = [self drawLandscape].CGPath;
            [self.delegate justDrawing];
            break;
...
-(void)preparing {
    ...
    newShapeLayer.strokeStart = 0.0;
    newShapeLayer.strokeEnd = 0.0;
    newShapeLayer.fillColor = UIColor.whiteColor.CGColor;
    newShapeLayer.strokeColor = UIColor.blackColor.CGColor;
    
    self.drawingLayer = newShapeLayer;
    [self.layer addSublayer:self.drawingLayer];
}

-(void)changeStrokeEnd {
    self.drawingLayer.strokeEnd += self.drawingSpeed;
}
